...
While a resource waits for a dependency, the plugin returns `StillProcessing`, which the framework turns into a plain requeue. The SECA resource stays in state `creating`. A plugin that wants the wait explained can return `RevisitBecause(d, cause)` instead: the cause reaches the log without flipping the state to `error`.
...
2. **The [`arubacloud-resource-operator`](https://github.com/Arubacloud/arubacloud-resource-operator) and its Aruba credentials** installed in that cluster — this is what actually provisions resources against the Aruba CMP. Install it with `make -C ../../test/conformance/aruba install-operator ARUBA_CLIENT_ID=<id> ARUBA_CLIENT_SECRET=<secret> ARUBA_TENANT=<account> KIND_CLUSTER=<cluster>`, or by hand from its [Helm chart](https://github.com/Arubacloud/helm-charts/tree/main/charts/arubacloud-resource-operator).
