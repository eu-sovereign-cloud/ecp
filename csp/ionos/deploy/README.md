...
2. Install the provider (replace PROVIDER_PKG with the provider package to install):
   ...
   PROVIDER_PKG="ionos-cloud/provider-upjet-ionoscloud:v0.5.9" ./install-provider.sh
...
- Convenience targets that wrap these scripts (`install-crossplane`, `install-provider`, `create-secret`, `provider-config`, `install-all`, `install-on-regional`, ...) live in `../Makefile`.
