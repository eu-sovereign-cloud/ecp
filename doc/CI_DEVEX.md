...
| `test/Makefile` | `test-all` (integration + e2e), `integration`, `integration-<component>`, `e2e`, `conformance`, `conformance-ionos`, `deploy-stack` (+ `kind-*` variants, one-shot on KIND), `build-all`, `push-all`, `deploy-all`, `kind-start`, `kind-stop`, `kind-load-all`, `bench`, `report` |
