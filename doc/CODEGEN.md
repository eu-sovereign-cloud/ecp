...
3. **CRD YAML** (`make generate-api`) — injects `+kubebuilder:validation:*` markers into every slice's generated schema and runs controller-gen `crd` over all 18 resource slices, emitting CRD YAML files into `charts/ecp/crds/`.
...
- `charts/ecp/crds/*.yaml` — 18 CRD YAML files (one per resource slice; see [CRD Generation](#crd-generation))
...
2. **Run controller-gen** — `go tool sigs.k8s.io/controller-tools/cmd/controller-gen crd` is invoked with
   ...
   `charts/ecp/crds/` (the repo-root CRD output directory).

**Output:** `charts/ecp/crds/*.yaml` — 18 flat CRD YAML files named `<group>_<plural>.yaml`:
...
`framework/backend/kubernetes/`, `resource/` (marker-laden schemas), and `charts/ecp/` (CRD YAML).
