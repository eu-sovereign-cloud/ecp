...
doesn't need one to run. The aruba plugin ships as its own single-plugin delegator
image (`delegator-aruba`, built from `csp/aruba/cmd`), so its conformance is the
**generic two-phase flow**. This folder is a placeholder that documents that flow
