...
// Compile-time assertion that *Nic satisfies common.Conditioned.
var _ common.Conditioned = (*Nic)(nil)

// GetConditions returns the Status.Conditions slice, or nil
// if the receiver, its Status or the slice is nil.
func (x *Nic) GetConditions() []types.StatusCondition {
...
func (x *Nic) PushCondition(condition types.StatusCondition) {
	if x == nil {
		return
	}

	if x.Status == nil {
		x.Status = &types.NicStatus{}
	}
...
// return nil.
func (x *Nic) PeekConditions() *types.StatusCondition {
	if x == nil || x.Status == nil || len(x.Status.Conditions) == 0 {
		return nil
	}
