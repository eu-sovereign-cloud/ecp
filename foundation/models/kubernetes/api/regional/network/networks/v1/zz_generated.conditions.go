...
// Compile-time assertion that *Network satisfies common.Conditioned.
var _ common.Conditioned = (*Network)(nil)

// GetConditions returns the Status.Conditions slice, or nil
// if the receiver, its Status or the slice is nil.
func (x *Network) GetConditions() []types.StatusCondition {
...
func (x *Network) PushCondition(condition types.StatusCondition) {
	if x == nil {
		return
	}

	if x.Status == nil {
		x.Status = &types.NetworkStatus{}
	}
...
// return nil.
func (x *Network) PeekConditions() *types.StatusCondition {
	if x == nil || x.Status == nil || len(x.Status.Conditions) == 0 {
		return nil
	}
