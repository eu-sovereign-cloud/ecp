...
func MapCRToNetworkDomain(obj client.Object) (*regional.NetworkDomain, error) {
	...
	spec := regional.NetworkSpecDomain{
		Cidr:          mapCRCidrToDomain(cr.Spec.Cidr),
		SkuRef:        mapCRReferenceToDomain(cr.Spec.SkuRef),
		RouteTableRef: mapCRReferenceToDomain(cr.Spec.RouteTableRef),
	}
	for _, c := range cr.Spec.AdditionalCidrs {
		spec.AdditionalCidrs = append(spec.AdditionalCidrs, mapCRCidrToDomain(c))
	}
...
func MapNetworkDomainToCR(domain *regional.NetworkDomain) (client.Object, error) {
	...
	for i, c := range domain.Spec.AdditionalCidrs {
		additionalCidrs[i] = mapDomainCidrToCR(c)
	}
	...
	cr := &networksv1.Network{
		...
		Spec: genv1.NetworkSpec{
			Cidr:            mapDomainCidrToCR(domain.Spec.Cidr),
			AdditionalCidrs: additionalCidrs,
			SkuRef:          mapDomainReferenceToCR(domain.Spec.SkuRef),
			RouteTableRef:   mapDomainReferenceToCR(domain.Spec.RouteTableRef),
		},
	}
...
// mapCRCidrToDomain converts a genv1.Cidr to a regional.CidrDomain.
func mapCRCidrToDomain(cr genv1.Cidr) regional.CidrDomain {
...
// mapDomainCidrToCR converts a regional.CidrDomain to a genv1.Cidr.
func mapDomainCidrToCR(domain regional.CidrDomain) genv1.Cidr {
	return genv1.Cidr{
		Ipv4: domain.IPv4,
		Ipv6: domain.IPv6,
	}
