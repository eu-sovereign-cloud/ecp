...
import (
	...
	globalcontroller "github.com/eu-sovereign-cloud/ecp/foundation/gateway/internal/controller/global"
)

// Region handles HTTP requests for region data.
type Region struct {
	Controller globalcontroller.RegionController
	Logger     *slog.Logger
}
...
func (h *Region) ListRegions(w http.ResponseWriter, r *http.Request, params regionv1.ListRegionsParams) {
	iterator, err := h.Controller.ListRegions(r.Context(), params)
	if err != nil {
...
func (h *Region) GetRegion(w http.ResponseWriter, r *http.Request, name schema.ResourcePathParam) {
	reg, err := h.Controller.GetRegion(r.Context(), name)
	if err != nil {
