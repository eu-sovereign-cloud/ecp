...
func CreateNamespace(ctx context.Context, clientSet kubernetes.Interface, name string, ownerLabels map[string]string) (created bool, err error) {
	...
	if _, err := clientSet.CoreV1().Namespaces().Create(ctx, ns, metav1.CreateOptions{}); err != nil {
		...
		return false, convertererrors.KubeToDomainError(fmt.Errorf("failed to create namespace %s: %w", name, err))
	}
...
func DeleteNamespace(ctx context.Context, clientSet kubernetes.Interface, name string) error {
	...
	if err := clientSet.CoreV1().Namespaces().Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
		...
		return convertererrors.KubeToDomainError(fmt.Errorf("failed to delete namespace %s: %w", name, err))
...
func (a *ReaderAdapter[T]) List(ctx context.Context, params model.ListParams, list *[]T) (*string, error) {
	...
	if err != nil {
		...
		return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to list resources for %s: %w", a.gvr.Resource, err))
	}
...
func (a *ReaderAdapter[T]) Load(ctx context.Context, obj *T) error {
	...
	if err != nil {
		...
		return convertererrors.KubeToDomainError(fmt.Errorf("failed to retrieve %s '%s': %w", a.gvr.Resource, v.GetName(), err))
	}
...
func (a *WriterAdapter[T]) Create(ctx context.Context, m T) (*T, error) {
	...
	if err != nil {
		...
		return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to create resource %s '%s': %w", a.gvr.Resource, m.GetName(), err))
	}
...
func (a *WriterAdapter[T]) Update(ctx context.Context, m T) (*T, error) {
	...
	if m.GetVersion() == "" {
		if err := a.updateMetadataAndSpecRetry(ctx, resourceInterface, m.GetName(), uobj); err != nil {
			return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to update metadata and spec %s '%s': %w", a.gvr.Resource, m.GetName(), err))
		}
	} else {
		if _, err = resourceInterface.Update(ctx, uobj, metav1.UpdateOptions{}); err != nil {
			return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to update metadata and spec with version %s %s '%s': %w", m.GetVersion(), a.gvr.Resource, m.GetName(), err))
		}
	}

	currObj, err := resourceInterface.Get(ctx, m.GetName(), metav1.GetOptions{})
	if err != nil {
		return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to get %s '%s' after update: %w", a.gvr.Resource, m.GetName(), err))
	}
...
func (a *WriterAdapter[T]) UpdateStatus(ctx context.Context, m T) (*T, error) {
	...
	if err := a.updateStatusRetry(ctx, resourceInterface, m, desiredStatus); err != nil {
		...
		return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to update status with retry %s '%s': %w", a.gvr.Resource, m.GetName(), err))
	}

	currObj, err := resourceInterface.Get(ctx, m.GetName(), metav1.GetOptions{})
	if err != nil {
		a.logger.ErrorContext(ctx, "failed to get resource after status update", "resource", a.gvr.Resource, "error", err)
		return nil, convertererrors.KubeToDomainError(fmt.Errorf("failed to get %s '%s' after status update: %w", a.gvr.Resource, m.GetName(), err))
	}
...
func (a *WriterAdapter[T]) Delete(ctx context.Context, m T) error {
	...
	if err != nil {
		...
		return convertererrors.KubeToDomainError(fmt.Errorf("failed to delete %s '%s': %w", a.gvr.Resource, m.GetName(), err))
	}
