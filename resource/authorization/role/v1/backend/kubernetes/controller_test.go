...
func TestRoleController_Reconcile(t *testing.T) {
	...
	newK8sResource := func() *Role {
		return &Role{
			...
			Spec:   RoleSpec{Permissions: []Permission{{Provider: "seca.compute", Resources: []string{"instances"}, Verb: []string{"get"}}}},
			Status: &RoleStatus{State: schemav1.ResourceStatePending},
		}
	}
	...
	t.Run("should successfully reconcile a valid resource", func(t *testing.T) {
		...
		handler := NewRolePluginHandler(mockRepo, mockPlugin, 1)
		gc := frameworkcontroller.NewGenericController[*roledom.Role](
			fakeClient,
			RoleFromCR,
			handler,
			&Role{},
