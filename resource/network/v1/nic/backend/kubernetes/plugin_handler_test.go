...
func TestNicPluginHandler_HandleReconcile(t *testing.T) {
	var (
	...
	t.Run("should do nothing if resource is active", func(t *testing.T) {
		...
		mockRepo := NewMockRepo[*nicdom.Nic](ctrl)
		mockPlugin := NewMockNicPlugin(ctrl)
		handler := NewNicPluginHandler(mockRepo, mockPlugin, 0)
	...
	t.Run("should set state to creating and requeue when resource is pending", func(t *testing.T) {
		...
		mockRepo.EXPECT().UpdateStatus(gomock.Any(), gomock.Any()).DoAndReturn(
			func(_ context.Context, res *nicdom.Nic) (*nicdom.Nic, error) {
	...
	t.Run("should fatal if state changes unexpectedly after delegation", func(t *testing.T) {
		if os.Getenv("BE_FATAL") == "1" {
			...
			mockPlugin.EXPECT().Create(gomock.Any(), gomock.Any()).DoAndReturn(
				func(_ context.Context, res *nicdom.Nic) error {
					res.Status.State = commondomain.ResourceStateActive
					return nil
				})

			handler := NewNicPluginHandler(NewMockRepo[*nicdom.Nic](ctrl), mockPlugin, 0)
			handler.HandleReconcile(context.Background(), resource) //nolint:errcheck
			return
		}

		cmd := exec.CommandContext(t.Context(), os.Args[0], "-test.run=TestNicPluginHandler_HandleReconcile/should_fatal_if_state_changes_unexpectedly_after_delegation")
