...
func TestSecurityGroupController_Reconcile(t *testing.T) {
	const (
		testName      = "test-sg"
	...
	newK8sResource := func() *SecurityGroup {
		return &SecurityGroup{
			...
			Status: &SecurityGroupStatus{State: schemav1.ResourceStatePending},
		}
	}
	...
	t.Run("should successfully reconcile a valid resource", func(t *testing.T) {
		...
		mockRepo := NewMockRepo[*securitygroupdom.SecurityGroup](mc)
		mockRepo.EXPECT().UpdateStatus(gomock.Any(), gomock.Any()).Return(nil, nil).Times(1)

		mockPlugin := NewMockSecurityGroupPlugin(mc)
		...
		handler := NewSecurityGroupPluginHandler(mockRepo, mockPlugin, 1)
		gc := frameworkcontroller.NewGenericController[*securitygroupdom.SecurityGroup](
			fakeClient,
			SecurityGroupFromCR,
			handler,
			&SecurityGroup{},
