...
func TestRouteTablePluginHandler_HandleReconcile(t *testing.T) {
	var (
	...
	t.Run("should call plugin update and write no status when resource is active and in sync", func(t *testing.T) {
		...
		resource := &routetabledom.RouteTable{
			Status: &routetabledom.RouteTableStatus{
		...
		mockRepo := NewMockRepo[*routetabledom.RouteTable](ctrl)
	...
	t.Run("should set state to creating and requeue when resource is pending", func(t *testing.T) {
		...
		mockRepo.EXPECT().UpdateStatus(gomock.Any(), gomock.Any()).DoAndReturn(
			func(_ context.Context, res *routetabledom.RouteTable) (*routetabledom.RouteTable, error) {
	...
	t.Run("should fatal if state changes unexpectedly after delegation", func(t *testing.T) {
		if os.Getenv("BE_FATAL") == "1" {
			...
			mockPlugin.EXPECT().Create(gomock.Any(), gomock.Any()).DoAndReturn(
				func(_ context.Context, res *routetabledom.RouteTable) error {
					res.Status.State = commondomain.ResourceStateActive
					return nil
				})

			handler := NewRouteTablePluginHandler(NewMockRepo[*routetabledom.RouteTable](ctrl), mockPlugin, 0)
			handler.HandleReconcile(context.Background(), resource) //nolint:errcheck
			return
		}

		cmd := exec.CommandContext(t.Context(), os.Args[0], "-test.run=TestRouteTablePluginHandler_HandleReconcile/should_fatal_if_state_changes_unexpectedly_after_delegation")
