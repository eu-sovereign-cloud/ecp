package kubernetes

import (
	"k8s.io/client-go/dynamic"
	"sigs.k8s.io/controller-runtime/pkg/client"

	k8sadapter "github.com/eu-sovereign-cloud/ecp/framework/backend/kubernetes"
	"github.com/eu-sovereign-cloud/ecp/framework/backend/kubernetes/builder"
	frameworkcontroller "github.com/eu-sovereign-cloud/ecp/framework/backend/kubernetes/controller"
	publicipdom "github.com/eu-sovereign-cloud/ecp/resource/network/v1/public-ip"
)

// Controller drives PublicIp reconciliation using the GenericController.
type Controller struct {
	frameworkcontroller.GenericController[*publicipdom.PublicIp]
}

// NewController wires together the PublicIp controller.
func NewController(
	ctrlClient client.Client,
	dynClient dynamic.Interface,
	plugin PublicIpPlugin,
	opts ...builder.Option,
) *Controller {
	options := builder.ApplyOptions(opts)
	repo := k8sadapter.NewRepoAdapter[*publicipdom.PublicIp](
		dynClient,
		PublicIPGVR,
		options.Logger,
		PublicIpToCR,
		PublicIpFromCR,
	)
	handler := NewPublicIpPluginHandler(repo, plugin, options.MaxConditions)
	return &Controller{
		GenericController: frameworkcontroller.NewGenericController[*publicipdom.PublicIp](
			ctrlClient,
			PublicIpFromCR,
			handler,
			&PublicIP{},
			options.RequeueAfter,
			options.Logger,
			options.MaxConditions,
		),
	}
}
