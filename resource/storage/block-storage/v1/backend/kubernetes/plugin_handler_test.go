...
import (
	...
	. "github.com/eu-sovereign-cloud/ecp/resource/storage/block-storage/v1/backend/kubernetes"
)
...
func TestBlockStoragePluginHandler_HandleReconcile(t *testing.T) {
	...
	t.Run("should set state to updating and requeue when size is increased on an active resource", func(t *testing.T) {
		...
		mockRepo.EXPECT().UpdateStatus(gomock.Any(), gomock.Any()).DoAndReturn(
			func(_ context.Context, res *bsdom.BlockStorage) (*bsdom.BlockStorage, error) {
				require.Equal(t, commondomain.ResourceStateUpdating, res.Status.State)
				return nil, nil
			}).Times(1)
	...
	t.Run("should fatal if state changes unexpectedly after delegation", func(t *testing.T) {
		if os.Getenv("BE_FATAL") == "1" {
			...
			mockPlugin.EXPECT().Create(gomock.Any(), gomock.Any()).DoAndReturn(
				func(ctx context.Context, res *bsdom.BlockStorage) error {
