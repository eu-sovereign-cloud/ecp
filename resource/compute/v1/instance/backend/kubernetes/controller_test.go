...
func TestInstanceController_Reconcile(t *testing.T) {
	const (
	...
	newK8sResource := func() *Instance {
		return &Instance{
			...
			Status: &InstanceStatus{State: schemav1.ResourceStatePending},
		}
	}
	...
	t.Run("should successfully reconcile a valid resource", func(t *testing.T) {
		...
		mockRepo := NewMockRepo[*instancedom.Instance](mc)
		mockRepo.EXPECT().UpdateStatus(gomock.Any(), gomock.Any()).Return(nil, nil).Times(1)

		mockPlugin := NewMockInstancePlugin(mc)
		...
		handler := NewInstancePluginHandler(mockRepo, mockPlugin, 1)
		gc := frameworkcontroller.NewGenericController[*instancedom.Instance](
			fakeClient,
			InstanceFromCR,
			handler,
			&Instance{},
